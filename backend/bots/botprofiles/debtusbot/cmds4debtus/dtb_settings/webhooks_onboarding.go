...
var claimInvite2 = func(ctx context.Context, inviteCode string, invite models4debtus.Invite, claimedByUserID, claimedOn, claimedVia string) error {
	return dal4debtus.Default.Invite.ClaimInvite2(ctx, inviteCode, invite, claimedByUserID, claimedOn, claimedVia)
}

var setMainMenuKeyboard = dtb_general.SetMainMenuKeyboard

func handleInviteOnStart(whc botsfw.WebhookContext, inviteCode string, invite models4debtus.Invite) (m botmsg.MessageFromBot, err error) {
	claimAndReply := func() {
		if err = claimInvite2(whc.Context(), inviteCode, invite, whc.AppUserID(), whc.BotPlatform().ID(), whc.GetBotCode()); err != nil {
	...
	if invite.Data.Related == InviteIsRelatedToOnboarding {
		if invite.Data.CreatedByUserID != whc.AppUserID() {
			return m, errors.New("invite.Related == InviteIsRelatedToOnboarding && invite.CreatedByUserID != whc.AppUserID()")
		}
		claimAndReply()
		return
	} else {
...
//func OnboardingTellAboutInviteCodeAction(whc botsfw.WebhookContext) (botmsg.MessageFromBot, error) {
...
//const TELL_ABOUT_INVITE_CODE_COMMAND = "tell_about_invite_code"
//
...
//}
//
//const ASK_INVITE_CHANNEL_COMMAND = "ask_invite_channel"
//
...
//}
//
//const ASK_INVITE_CODE_COMMAND = "ask_invite_code"
//
...
//}
//
//const CHECK_INVITE_COMMAND = "check_invite"
...
//
//		if err = dal4debtus.Default.Invite.ClaimInvite(ctx, userID, inviteCode, whc.BotPlatform().ContactID(), whc.GetBotCode()); err != nil {
...
func TextCommand(on string, message []string, icon, replyIcon string, hideKeyboard bool) botsfw.Command {
	return botsfw.Command{
		Code:       botsfw.CommandCode(fmt.Sprintf("TextCommand(%v)", on)),
		Title:      on,
		Icon:       icon,
		ExactMatch: on,
		Action: func(whc botsfw.WebhookContext) (botmsg.MessageFromBot, error) {
...
var OnboardingOnUserContactReceivedCommand = botsfw.Command{
	Code: "on_user_contact_received",
	Action: func(whc botsfw.WebhookContext) (m botmsg.MessageFromBot, err error) {
		return
	},
}

//func onboardingProcessPhoneContact(whc botsfw.WebhookContext, contact botsfw.WebhookContactMessage) (m botmsg.MessageFromBot, err error) {
//	ctx := whc.Context()
//	//whc.ChatData().SetAwaitingReplyTo(OnUserContactReceivedCommandCode)
//	invite, err := dal4debtus.Default.Invite.CreatePersonalInvite(whc.ExecutionContext(), whc.AppUserID(), models.InviteBySms, contact.PhoneNumber(), whc.BotPlatform().ContactID(), whc.GetBotCode(), InviteIsRelatedToOnboarding)
//	if err != nil {
//		return m, err
//	}
//
...
const EmailConfirmationSentCommandCode = "email_confirmation_sent"

const InviteIsRelatedToOnboarding = "onboarding=yes"

//func onboardingProcessEmail(messageText string, whc botsfw.WebhookContext, altCmd ContactsChannelCommand) (m botmsg.MessageFromBot, err error) {
//
...
//		invite, err := dal4debtus.Default.Invite.CreatePersonalInvite(whc.ExecutionContext(), whc.AppUserID(), models.InviteByEmail, email, whc.BotPlatform().ContactID(), whc.GetBotCode(), InviteIsRelatedToOnboarding)
//		if err != nil {
//			return m, err
//		}
//		_, err = invites.SendInviteByEmail(whc.ExecutionContext(), "Debtus", email, whc.GetSender().GetFirstName(), invite.ContactID, whc.GetBotCode(), anybot.UtmSourceFromContext(whc))
//		if err != nil {
//			return m, err
//		}
//		whc.ChatData().SetAwaitingReplyTo(EmailConfirmationSentCommandCode)
