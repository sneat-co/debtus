...
const (
	...
	transferWizardParamComment = "comment"
)

//const (
//	ADD_NOTE_COMMAND    = "add_note"
//	ADD_COMMENT_COMMAND = "add_comment"
//)
//
...
//					if chatEntity.GetWizardParam(transferWizardParamComment) != "" {
...
//					)
//				case ADD_COMMENT_COMMAND:
//					chatEntity.AddWizardParam(transferWizardParamComment, whc.Input().(botsfw.WebhookTextMessage).Text())
...
//					panic(fmt.Sprintf("Unknown code: %v", code))
//				}
//				chatEntity.PopStepsFromAwaitingReplyUpToSpecificParent(askNoteOrCommentForTransferCommandCode)
//			} else {
...
func createTransferAskNoteOrCommentCommand(code botsfw.CommandCode, nextCommand botsfw.Command) botsfw.Command {
		...
		Action: func(whc botsfw.WebhookContext) (m botmsg.MessageFromBot, err error) {
			...
			if chatEntity.IsAwaitingReplyTo(string(code)) {
				...
				case whc.Translate(trans.COMMAND_TEXT_NO_COMMENT_FOR_TRANSFER):
					...
					//	return nextCommand.Action(whc)
				default:
					chatEntity.AddWizardParam(transferWizardParamComment, mt)
					return nextCommand.Action(whc)
			...
			m.Keyboard = tgbotapi.NewInlineKeyboardMarkup(
				[]tgbotapi.InlineKeyboardButton{
					tgbotapi.NewInlineKeyboardButtonData(whc.Translate(trans.COMMAND_TEXT_MORE_ABOUT_INTEREST_COMMAND), askForInterestAndCommentCallbackCommandCode),
				},
			)
...
const askForInterestAndCommentCallbackCommandCode = "ask_for_interest_and_comment_long"

var askForInterestAndCommentCallbackCommand = botsfw.Command{
	Code: askForInterestAndCommentCallbackCommandCode,
...
const askNoteOrCommentForTransferCommandCode = "ask_note_or_comment"

var transferFromUserAskNoteOrCommentCommand = createTransferAskNoteOrCommentCommand(
	askNoteOrCommentForTransferCommandCode,
