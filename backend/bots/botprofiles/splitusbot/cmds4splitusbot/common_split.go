...
) (m botmsg.MessageFromBot, err error) {
	ctx := whc.Context()
...
func writeSplitInstructions(buffer *bytes.Buffer, tgUserID, memberName string) {
	...
	buffer.WriteString("\n\n")
	if tgUserID == "" {
		_, _ = fmt.Fprintf(buffer, "<b>Selected:</b> %v", memberName)
	} else {
		_, _ = fmt.Fprintf(buffer, `<b>Selected:</b> <a href="tg://user?id=%v">%v</a>`, tgUserID, memberName)
	}
}
...
func writeSplitMembers(buffer *bytes.Buffer, members []*briefs4splitus.BillMemberBrief, currentMemberID string, currency money.CurrencyCode) {
	...
	for i, m := range members {
		if m.ID == currentMemberID {
			_, _ = fmt.Fprintf(buffer, "  <b>%d. %v</b>\n", i+1, html.EscapeString(m.Name))
		} else {
			_, _ = fmt.Fprintf(buffer, "  %d. %v\n", i+1, html.EscapeString(m.Name))
		}
		_, _ = fmt.Fprintf(buffer, "     <i>Shares: %d</i> — <code>%v%%</code>", m.Shares, decimal.Decimal64p2(m.Shares*100*100/totalShares))
		if m.Owes != 0 {
			_, _ = buffer.WriteString(" = " + money.Amount{Currency: currency, Value: m.Owes}.String())
		}
