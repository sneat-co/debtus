...
const billCardCommandCode = "bill_card"

var billCardCommand = botsfw.Command{
	Code:       billCardCommandCode,
	InputTypes: []botinput.Type{botinput.TypeCallbackQuery},
	CallbackAction: billCallbackAction(func(whc botsfw.WebhookContext, _ dal.ReadwriteTransaction, callbackUrl *url.URL, bill models4splitus.BillEntry) (m botmsg.MessageFromBot, err error) {
		ctx := whc.Context()
...
func startBillAction(whc botsfw.WebhookContext, billParam string) (m botmsg.MessageFromBot, err error) {
...
const billMembersCommandCode = "bill_members"
...
func ShowBillCard(whc botsfw.WebhookContext, isEdit bool, bill models4splitus.BillEntry, footer string) (m botmsg.MessageFromBot, err error) {
	ctx := whc.Context()
	m = whc.NewMessage("")
	m.IsEdit = isEdit
...
func writeBillCardTitle(ctx context.Context, bill models4splitus.BillEntry, botID string, buffer *bytes.Buffer, translator i18n.SingleLocaleTranslator) error {
	...
	titleWithLink := fmt.Sprintf(`<a href="%s">%v</a>`, bothelper.StartTelegramBotUrl(botID, "bill", "id="+bill.ID), bill.Data.Name)
