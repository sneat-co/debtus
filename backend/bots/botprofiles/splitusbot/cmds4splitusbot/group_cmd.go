...
var groupCommand = botsfw.NewCallbackCommand(groupCommandCode,
	func(whc botsfw.WebhookContext, callbackUrl *url.URL) (m botmsg.MessageFromBot, err error) {
		...
		_ = callbackUrl
		m.Text = "Sorry, the group card is not available yet."
		return
		...
		//
		//if group, err = dal4debtus.Group.GetGroupByID(c, nil, userGroupJson.ContactID); err != nil {
		//	return
		//}
		//
		//buf := new(bytes.Buffer)
		//
		//_, _ = fmt.Fprintf(buf, "<b>Group #%d</b>: %v", i+1, userGroupJson.UserTitle)
