...
var sendInviteByEmail = email4debtus.SendInviteByEmail
	...
	return botsfw.Command{
		...
		Action: func(whc botsfw.WebhookContext) (m botmsg.MessageFromBot, err error) {
			...
			if chatEntity.IsAwaitingReplyTo(code) {
				email := strings.TrimSpace(whc.Input().(botinput.TextMessage).Text())
				isValid := channel == string(models4debtus.InviteByEmail) && strings.Contains(email, "@") && strings.Contains(email, ".")
				if isValid {
					invite, err := dal4debtus.Default.Invite.CreatePersonalInvite(whc, whc.AppUserID(), models4debtus.InviteByEmail, email, whc.BotPlatform().ID(), whc.GetBotCode(), "counterparty=?")
					if err != nil {
						logus.Errorf(whc.Context(), "Failed to call invites.CreateInvite()")
						return m, err
					}
					var emailID string
					emailID, err = sendInviteByEmail(
						...
						invite.ID,
						whc.GetBotCode(),
						utm4bots.UtmSourceFromContext(whc),
					)
					if err != nil {
						return m, err
					}
