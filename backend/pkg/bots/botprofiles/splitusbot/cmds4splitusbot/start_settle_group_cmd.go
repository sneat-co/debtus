...
const (
	SettleGroupAskForCounterpartyCommandCode    = "sttl_grp"
	SettleGroupCounterpartyChosenCommandCode    = "sttl_grp_cp_chsn"
	SettleGroupCounterpartyConfirmedCommandCode = "sttl_grp_cp_cnfrmd"
)
...
var settleGroupAskForCounterpartyCommand = botsfw.Command{
	...
	Action: bothelper.NewSpaceAction(func(whc botsfw.WebhookContext, space dbo4spaceus.SpaceEntry) (m botmsg.MessageFromBot, err error) {
...
func settleGroupStartAction(whc botsfw.WebhookContext, startParams []string) (m botmsg.MessageFromBot, err error) {
	var space dbo4spaceus.SpaceEntry
	for _, p := range startParams {
		switch {
		case strings.HasPrefix(p, "space="):
			space.ID = coretypes.SpaceID(p[len("space="):])
		}
	}
	//if space, err = dal4debtus.Group.GetGroupByID(whc.Context(), nil, space.ContactID); err != nil {
	//	return
	//}
	return settleGroupAskForCounterpartyAction(whc, space)
}

func settleGroupAskForCounterpartyAction(_ botsfw.WebhookContext, _ dbo4spaceus.SpaceEntry) (m botmsg.MessageFromBot, err error) {
	...
	//		buf.WriteString(whc.Translate(trans.MT_GROUP_LABEL, space.Data.UserTitle) + "\n\n")
	//
	//		switch {
...
var settleGroupCounterpartyChosenCommand = bothelper.NewSpaceCallbackCommand(
...
func settleGroupCounterpartyChosenAction(whc botsfw.WebhookContext, space dbo4spaceus.SpaceEntry, memberID string) (m botmsg.MessageFromBot, err error) {
	err = errors.New("not implemented yet")
	return
	...
	//	return
	//}
	//m.Text = fmt.Sprintf("Have you returned this debt to %v already or you will return it?", counterpartyMember.UserTitle)
	//
...
var settleGroupCounterpartyConfirmedCommand = bothelper.NewSpaceCallbackCommand(
...
func settleGroupCounterpartyConfirmedAction(whc botsfw.WebhookContext, space dbo4spaceus.SpaceEntry, memberID string, currency money.CurrencyCode) (m botmsg.MessageFromBot, err error) {
	err = errors.New("not implemented yet")
	return
	//var userMember, counterpartyMember briefs4splitus.GroupMemberJson
	//
