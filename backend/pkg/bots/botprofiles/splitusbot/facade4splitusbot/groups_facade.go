...
func CreateGroup(ctx context.Context,
	...
	if err = facade.RunReadwriteTransaction(ctx, func(ctx context.Context, tx dal.ReadwriteTransaction) error {
		...
		//		if existingGroup.UserTitle == groupEntity.UserTitle {
		//			return errors.New("Duplicate group name")
		//		}
		//	}
		//	if group, err = dal4debtus.Group.InsertGroup(ctx, tx, groupEntity); err != nil {
		//		return err
		//	}
		//} else if groupMembersChanged {
		//	if err = dal4debtus.Group.SaveGroup(ctx, tx, group); err != nil {
		//		return err
		//	}
		//}
		//
...
type NewUser struct {
	...
	ChatMember botinput.Actor
}

func AddUsersToTheGroupAndOutstandingBills(ctx context.Context, spaceID coretypes.SpaceID, newUsers []NewUser) (splitusSpace models4splitus.SplitusSpaceEntry, newUsers2 []NewUser, err error) {
	...
	if len(newUsers) == 0 {
		err = errors.New("AddUsersToTheGroupAndOutstandingBills: len(newUsers) == 0")
		return
	}
...
func DelayUpdateGroupUsers(ctx context.Context, groupID string) error { // TODO: Move to DAL?
	...
	return facade4splitus.DelayerUpdateGroupUsers.EnqueueWork(ctx, delaying.With(const4userus.QueueUsers, "update-group-users", 0), groupID)
}

func delayedUpdateGroupUsers(ctx context.Context, spaceID coretypes.SpaceID) (err error) {
	if spaceID == "" {
	...
	if err = facade.RunReadwriteTransaction(ctx, func(ctx context.Context, tx dal.ReadwriteTransaction) (err error) {
		...
		for _, member := range splitusSpace.Data.GetGroupMembers() {
			if member.UserID != "" {
				if err = delayUpdateUserWithGroups(ctx, member.UserID, []string{string(spaceID)}, []string{}); err != nil {
					return err
				}
			}
		}
...
func delayedUpdateUserWithGroups(ctx context.Context, userID string, groupIDs2add, groupIDs2remove []string) (err error) {
	...
	if err = facade.RunReadwriteTransaction(ctx, func(ctx context.Context, tx dal.ReadwriteTransaction) (err error) {
		...
		for i, spaceID := range groupIDs2add {
			groups2add[i] = models4splitus.NewSplitusSpaceEntry(coretypes.SpaceID(spaceID))
		}
...
func DelayUpdateContactWithGroups(ctx context.Context, contactID string, addGroupIDs, removeGroupIDs []string) error {
	return facade4splitus.DelayerUpdateContactWithGroups.EnqueueWork(ctx, delaying.With(const4userus.QueueUsers, "update-contact-groups", 0), contactID, addGroupIDs, removeGroupIDs)
}
...
//			return
//		}
//		//if splitusSpace, err = dal4debtus.GroupEntry.GetGroupByID(c, groupID); err != nil {
//		//	return
//		//}
//		//if user, err = dal4debtus.UserEntry.GetUserByStrID(c, userID); err != nil {
//		//	return
//		//}
//
//		{ // Update splitusSpace
