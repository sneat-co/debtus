...
var parseTransferCommand = botsfw.Command{
	...
	Action: func(whc botsfw.WebhookContext) (m botmsg.MessageFromBot, err error) {
		match := transferRegex.FindStringSubmatch(whc.Input().(botinput.TextMessage).Text())
		...
		fmt.Fprintf(&buffer, "You've %v %v %v %v %v", verb, valueS, currency, direction, counterpartyName)
		if when != "" {
			//TODO: Convert to time.ID
