...
func createStartTransferWizardCommand(code botsfw.CommandCode, messageText string, commands []string, askTransferAmountCommand botsfw.Command) botsfw.Command {
	return botsfw.Command{
		...
		Matcher: func(c botsfw.Command, whc botsfw.WebhookContext) bool {
			if m, ok := whc.Input().(botinput.TextMessage); ok && IsCurrencyIcon(m.Text()) && whc.ChatData().GetAwaitingReplyTo() == string(code) {
				return true
			}
			return false
		},
		Action: func(whc botsfw.WebhookContext) (m botmsg.MessageFromBot, err error) {
			...
			default:
				...
				if isMainMenuCommand {
					whc.ChatData().SetAwaitingReplyTo(string(code))
...
var AskLendingAmountCommand = AskTransferAmountCommand("ask-lending-amount", trans.MESSAGE_TEXT_ASK_LENDING_AMOUNT,
	CreateAskTransferCounterpartyCommand(false, "ask-lending-counterparty", "", "", trans.MESSAGE_TEXT_ASK_LENDING_COUNTERPARTY,
		[]botsfw.Command{transferAskDueDateReturnToUserCommand},
		newCounterpartyCommand(transferAskDueDateReturnToUserCommand),
		_transferAskDueDate(transferAskDueDateReturnToUserCommand),
	),
)
...
var AskBorrowingAmountCommand = AskTransferAmountCommand("ask-borrowing-amount", trans.MESSAGE_TEXT_ASK_BORROWING_AMOUNT,
	CreateAskTransferCounterpartyCommand(false, "ask-borrowing-counterparty", "", "", trans.MESSAGE_TEXT_ASK_BORROWING_COUNTERPARTY,
		[]botsfw.Command{transferAskDueDateReturnByUserCommand},
		newCounterpartyCommand(transferAskDueDateReturnByUserCommand),
		_transferAskDueDate(transferAskDueDateReturnByUserCommand),
	),
)
...
const setDueDateCommandCode = "set_due_date"

var SetDueDateCommand = botsfw.Command{
	Code: setDueDateCommandCode,
...
func _transferAskDueDate(c botsfw.Command) _onContactSelectedAction {
	return func(whc botsfw.WebhookContext, counterparty models4debtus.DebtusSpaceContactEntry) (m botmsg.MessageFromBot, err error) {
		return c.Action(whc)
	}
}
...
func transferAskDueDateCommand(code botsfw.CommandCode, nextCommand botsfw.Command) botsfw.Command {
	return botsfw.Command{
		Code: code,
		...
		Action: func(whc botsfw.WebhookContext) (botmsg.MessageFromBot, error) {
			...
			chatEntity := whc.ChatData()
			if chatEntity.IsAwaitingReplyTo(string(code)) {
				mt := strings.TrimSpace(whc.Input().(botinput.TextMessage).Text())
...
var transferAskDueDateReturnByUserCommand = transferAskDueDateCommand(
	"ask_due_date_return_by_user",
	transferFromUserAskNoteOrCommentCommand,
)
var transferAskDueDateReturnToUserCommand = transferAskDueDateCommand(
	"ask_due_date_return_to_user",
	TransferToUserAskNoteOrCommentCommand,
)

func processSetDate(whc botsfw.WebhookContext) (m botmsg.MessageFromBot, date time.Time, err error) {
	ctx := whc.Context()
