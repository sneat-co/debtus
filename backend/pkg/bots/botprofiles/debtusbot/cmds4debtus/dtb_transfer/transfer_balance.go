...
// balanceWithInterestFn is a seam allowing tests to inject an error from BalanceWithInterest.
var balanceWithInterestFn = func(o *models4debtus.DebtusContactBrief, ctx context.Context, now time.Time) (money.Balance, error) {
	return o.BalanceWithInterest(ctx, now)
}
...
func (m BalanceMessageBuilder) ByContact(
	...
	for contactID, debtusContactBrief := range debtusContactBriefs {
		counterpartyBalanceWithInterest, err := balanceWithInterestFn(debtusContactBrief, ctx, now)
		if err != nil {
	...
	sort.Slice(sortedContactBriefs, func(i, j int) bool {
		b1 := sortedContactBriefs[i].Balance
		b2 := sortedContactBriefs[j].Balance
	...
	//	logus.Debugf(ctx, "There are %d debtusContactBriefs with zero balance: %v", counterpartiesWithZeroBalanceCount, strings.TrimRight(counterpartiesWithZeroBalance.CallbackData(), ", "))
	//}
