...
import (
	...
	"github.com/sneat-co/debtus/backend/pkg/bots/botprofiles/debtusbot/dtb_common"
...
var remindAgainCallbackCommand = botsfw.NewCallbackCommand(dtb_common.CallbackRemindAgain,
...
func rescheduleReminder(whc botsfw.WebhookContext, reminderID string, remindInDuration time.Duration) (m botmsg.MessageFromBot, err error) {
	ctx := whc.Context()

	var oldReminder, newReminder dbo4reminders.Reminder

	if oldReminder, newReminder, err = dal4reminders.RescheduleReminder(ctx, reminderID, remindInDuration); err != nil {
		if errors.Is(err, dal4debtus.ErrReminderAlreadyRescheduled) {
	...
	if transfer, err = facade4debtus.Transfers.GetTransferByID(ctx, nil, oldReminder.Data.TargetID); err != nil {
	...
	if chatEntity.IsAwaitingReplyTo(setNextReminderDateCommandCode) {
...
//const ASK_FOR_FEEDBACK_TASK = "ask4feedback"
//
...
//func disableReminders(whc botsfw.WebhookContext, transferID int) (m botmsg.MessageFromBot, err error) {
//	ctx := whc.Context()
//	transferKey, transfer, err := facade4debtus.Transfers.GetTransferByID(c, transferID)
//	userID := whc.AppUserID()
//	if !transfer.IsRemindersDisabled(userID) {
//		err = dal4debtus.DB.RunInTransaction(ctx, func(tctx context.Context) error {
