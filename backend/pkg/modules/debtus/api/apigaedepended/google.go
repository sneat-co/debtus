...
func InitApiGaeDepended() {
	handleFunc("/auth/google/signin", dal4debtus.HttpAppHost.HandleWithContext(handleSigninWithGoogle))
	handleFunc("/auth/google/signed", dal4debtus.HttpAppHost.HandleWithContext(handleSignedWithGoogle))
}
...
func handleSignedWithGoogle(_ context.Context, w http.ResponseWriter, _ *http.Request) {
	http.Error(w, "disabled: switch to Firestore authentication", http.StatusNotImplemented)
	//var userID string
