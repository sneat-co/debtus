...
import (
	...
	"github.com/sneat-co/sneat-go/pkg/modules/debtus/email4debtus"
...
func HandleGetReceipt(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	if err = facade.RunReadwriteTransaction(ctx, func(ctx context.Context, tx dal.ReadwriteTransaction) (err error) {
		transfer, err = getTransferByIDFn(ctx, tx, receipt.Data.TransferID)
		...
		if err = checkTransferCreatorNameFn(ctx, tx, transfer); common4all.HasError(ctx, w, err, models4debtus.TransfersCollection, receipt.Data.TransferID, http.StatusInternalServerError) {
			return
		}
		return nil
	}); err != nil {
	...
	if receipt.Data.SentVia == "telegram" {
		...
		sentTo, err = facade2bots.GetBotID(env, "telegram", debtusbotconst.ProfileID, lang)
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
...
func HandleSendReceipt(ctx context.Context, w http.ResponseWriter, r *http.Request, authInfo token4auth.AuthInfo, user dbo4userus.UserEntry) {
	...
	transfer, err := getTransferByIDFn(ctx, nil, receipt.Data.TransferID)
	if err != nil {
	...
	if _, err = sendReceiptByEmailFn(ctx, translator, receipt, user.Data.GetFullName(), transfer.Data.Counterparty().ContactName, toAddress); err != nil {
...
func updateReceiptAndTransferOnSent(ctx context.Context, receiptID string, channel, sentTo, lang string) (receipt models4debtus.ReceiptEntry, transfer models4debtus.TransferEntry, err error) {
	...
	err = facade.RunReadwriteTransaction(ctx, func(ctx context.Context, tx dal.ReadwriteTransaction) error {
		...
		if receipt.Data.SentVia == RECEIPT_CHANNEL_DRAFT {
			if transfer, err = getTransferByIDFn(ctx, tx, receipt.Data.TransferID); err != nil {
				return err
			}
...
var (
	...
	sendReceiptByEmailFn       = email4debtus.SendReceiptByEmail
	renderReceiptTemplateFn    = func(ctx context.Context, translator i18n.SingleLocaleTranslator, params any) (string, error) {
		return common4all.TextTemplates.RenderTemplate(ctx, translator, trans.MESSENGER_RECEIPT_TEXT, params)
	}
)

// getReceiptChannel is a seam — overridable in tests to drive the
// non-errUnknownChannel error branch of HandleSetReceiptChannel.
...
func HandleCreateReceipt(ctx context.Context, w http.ResponseWriter, r *http.Request, authInfo token4auth.AuthInfo) {
	...
	transfer, err := getTransferByIDFn(ctx, nil, transferID)
	if err != nil {
		if dal.IsNotFound(err) {
			w.WriteHeader(http.StatusBadRequest)
		} else {
	...
	if channel == "telegram" {
		...
		messageToSend, err = renderReceiptTemplateFn(ctx, translator, templateParams) // TODO: Consider using just ExecutionContext instead of (ctx, translator)
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
