...
func TestUserContactJson_BalanceWithInterest(t *testing.T) {
	...
	data, err := json.Marshal(debtusContactBrief)
	if err != nil {
		t.Fatal(err)
	}
	err = json.Unmarshal(data, &debtusContactBrief)
	if err != nil {
		t.Fatal(err)
	}
...
// TestCalculateInterestValue_UnknownFormula_Panic covers the panic branch in
// calculateInterestValue (interests_transfer.go:164-165).
// interest.Calculate returns an error for any unknown formula, which triggers the panic.
// We use COVER-BEFORE-PANIC: wrap in recover() so the test doesn't fail.
func TestCalculateInterestValue_UnknownFormula_Panic(t *testing.T) {
	defer func() {
		if r := recover(); r == nil {
			t.Error("expected panic for unknown interest formula")
		}
	}()
	td := &TransferData{
		AmountInCents: 10000,
		DtCreated:     time.Now().Add(-7 * 24 * time.Hour),
		TransferInterest: TransferInterest{
			InterestType:    "compound", // unknown formula — triggers error in interest.Calculate
			InterestPercent: decimal.NewDecimal64p2FromInt(5),
			InterestPeriod:  7,
		},
	}
	// GetInterestValue skips the early-return because InterestType!="" or InterestPeriod!=0
	// then calls calculateInterestValue which calls interest.Calculate with unknown formula
	// → error returned → panic(err)
	_ = td.GetInterestValue(time.Now())
}
