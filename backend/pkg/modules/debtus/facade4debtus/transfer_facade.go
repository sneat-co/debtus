...
func (transferFacade TransfersFacade) checkOutstandingTransfersForReturns(ctx context.Context, now time.Time, input CreateTransferInput) (returnToTransferIDs []string, err error) {
	...
	outstandingTransfers, err = dal4debtus.Default.Transfer.LoadOutstandingTransfers(ctx, db, now, creatorUserID, creatorContactID, input.Request.Amount.Currency, reversedDirection)
	if err != nil {
	...
	logus.Debugf(ctx, "facade4debtus.checkOutstandingTransfersForReturns() => dal4debtus.TransferEntry.LoadOutstandingTransfers(userID=%v, currency=%v) => %d api4transfers", input.CreatorUser.ID, input.Request.Amount.Currency, len(outstandingTransfers))
...
func (transferFacade TransfersFacade) createTransferWithinTransaction(
	...
	// In case if we just loaded above missing counterparty we need to check for missing user
	{
		loadUserIfNeeded := func(who string, appUser dbo4userus.UserEntry) (dbo4userus.UserEntry, error) {
			logus.Debugf(ctx, "%v.UserID: %s, %sUser.Data: %+v", who, appUser.ID, who, appUser.Data)
			if appUser.ID != "" {
				if appUser.Data == nil {
	...
	if len(transferReturnInfos) > 2 {
		transferReturnUpdates := make([]dal4debtus.TransferReturnUpdate, len(transferReturnInfos))
		for i, tri := range transferReturnInfos {
			transferReturnUpdates[i] = dal4debtus.TransferReturnUpdate{TransferID: tri.Transfer.ID, ReturnedAmount: tri.ReturnedAmount}
		}
		if err = dal4debtus.Default.Transfer.DelayUpdateTransfersOnReturn(ctx, createdTransfer.ID, transferReturnUpdates); err != nil {
			return
		}
	} else {
	...
	if output.Transfer.Data.Counterparty().UserID != "" {
		if err = dal4debtus.Default.Receipt.DelayCreateAndSendReceiptToCounterpartyByTelegram(ctx, input.Env, createdTransfer.ID, createdTransfer.Data.Counterparty().UserID); err != nil {
	...
	if createdTransfer.Data.IsOutstanding && dal4debtus.Default.Reminder != nil { // TODO: check for nil is temporary workaround for unittest
		if err = dal4debtus.Default.Reminder.DelayCreateReminderForTransferUser(ctx, createdTransfer.ID, createdTransfer.Data.CreatorUserID); err != nil {
...
func InsertTransfer(ctx context.Context, tx dal.ReadwriteTransaction, transferEntity *models4debtus.TransferData) (transfer models4debtus.TransferEntry, err error) {
	transfer = models4debtus.NewTransferWithIncompleteKey(transferEntity)
	if err = tx.Insert(ctx, transfer.Record, dal.WithAdapterGeneratedID()); err != nil {
		return
	}
	transfer.ID = transfer.Record.Key().ID.(string)
	return
}
...
func (TransfersFacade) UpdateTransferOnReturn(ctx context.Context, tx dal.ReadwriteTransaction, returnTransfer, transfer models4debtus.TransferEntry, returnedAmount decimal.Decimal64p2) (err error) {
	...
	if dal4debtus.Default.Reminder != nil {
		if err = dal4debtus.Default.Reminder.DelayDiscardRemindersForTransfers(ctx, []string{transfer.ID}, returnTransfer.ID); err != nil {
