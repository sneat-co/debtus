...
func delayedUpdateUserWithBill(ctx context.Context, billID, userID string) (err error) {
	...
	var (
		...
		wg                sync.WaitGroup
		billErr           error
		UserExtDboChanged bool
		userIsBillMember  bool
	)
	wg.Add(1)
	go func() {
		defer wg.Done()
		if bill, billErr = getBillEntryByID(ctx, nil, billID); err != nil {
			return
		}
	}()
	...
	if err = dal4userus.RunUserExtWorker(ctx, userID, const4debtus.ModuleID, new(models4splitus.SplitusUserDbo),
		func(ctx context.Context, tx dal.ReadwriteTransaction, params *dal4userus.UserExtWorkerParams[models4splitus.SplitusUserDbo]) (err error) {
			...
			userOutstandingBills := params.UserExt.Data.GetOutstandingBills()
			...
			if UserExtDboChanged {
				if err = setUserOutstandingBills(params.UserExt.Data, userOutstandingBills); err != nil {
					return
				}

				if err = tx.Set(ctx, params.UserExt.Record); err != nil {
					return
				}
			} else {
