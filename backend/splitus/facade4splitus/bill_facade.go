...
func AssignBillToGroup(ctx context.Context, tx dal.ReadwriteTransaction, inBill models4splitus.BillEntry, spaceID coretypes.SpaceID, userID string) (bill models4splitus.BillEntry, splitusSpace models4splitus.SplitusSpaceEntry, err error) {
	bill = inBill
	if err = bill.Data.AssignToGroup(string(spaceID)); err != nil {
		return
	}
	...
	if len(bill.Data.Members) == 0 {
		...
		if len(splitusSpace.Data.Members) > 0 {
			...
			if bill.Data.Currency != money.CurrencyCode("") {
				if _, err = applyBillBalanceDifference(splitusSpace.Data, bill.Data.Currency, bill.Data.GetBalance().BillBalanceDifference(briefs4splitus.BillBalanceByMember{})); err != nil {
					return
				}
...
func CreateBill(ctx context.Context, tx dal.ReadwriteTransaction, spaceID coretypes.SpaceID, billEntity *models4splitus.BillDbo) (bill models4splitus.BillEntry, err error) {
	if ctx == nil {
	...
	if !isValidBillSplit(billEntity.SplitMode) {
		err = fmt.Errorf("%w: billEntity.SplitMode has unknown value: %v", errors4debtus.ErrBadInput, billEntity.SplitMode)
		return
	}
	...
	if billEntity.Status == "" {
		err = fmt.Errorf("%w: billEntity.Status property is required", errors4debtus.ErrBadInput)
		return
	}
	...
	if len(members) == 0 {
					...
					if member.UserID == "" {
						...
						for _, counterparty := range member.ContactByUser {
							if counterparty.ContactID == "" {
								err = fmt.Errorf("%w: members[%d] has ContactByUser entry with empty ContactID", errors4debtus.ErrBadInput, i)
								return
							}
		...
		for i, member := range members {
			...
			ensureNoAdjustment := func() error {
				if member.Adjustment != 0 {
					return fmt.Errorf("%w: member #%d has Adjustment property not allowed with split mode %v", errors4debtus.ErrBadInput, i, billEntity.SplitMode)
				}
				return nil
			}
			ensureNoShare := func() error {
				if member.Shares != 0 {
					return fmt.Errorf("%w: member #%d has Shares property not allowed with split mode %v", errors4debtus.ErrBadInput, i, billEntity.SplitMode)
				}
				return nil
			}
			ensureEqualShare := func() error {
				if member.Shares != members[0].Shares {
					return fmt.Errorf("%w: members[%d] has Shares not equal to members[0].Shares: %d != %d", errors4debtus.ErrBadInput, i, member.Shares, members[i].Shares)
				}
				return nil
			}
			...
			case models4splitus.SplitModeEqually:
				// ensureNoAdjustment()
				if err = ensureEqualShare(); err != nil {
					return
				}
			...
			case models4splitus.SplitModeExactAmount:
				if err = ensureNoAdjustment(); err != nil {
					return
				}
				if err = ensureNoShare(); err != nil {
					return
				}
		...
		if (billEntity.Status != const4debtus.StatusDraft || totalPaidByMembers != 0) &&
			totalPaidByMembers != billEntity.AmountTotal {
	...
	if err = dal4debtus.InsertWithRandomStringID(ctx, tx, billHistory.Record); err != nil {
		return
	}
	return
}
...
//	err := dal4debtus.DB.RunInTransaction(ctx, func(ctx context.Context) error {
//		bill, err := facade4debtus.GetBillByID(ctx, billID)
//
//		if err != nil {
//			return err
...
//			return ErrBillHasNoPayer
//		}
//		//transferSource := dal4debtus.NewTransferSourceBot("api4debtus", "no-id", "0") // TODO: Needs refactoring! Move it out of DAL, do we really need an interface?
//
...
//		//}
//		return nil
//	}, dal4debtus.CrossGroupTransaction)
//	return err
//}
...
func AddBillMember(
	...
	if paid < 0 {
		err = fmt.Errorf("%w: paid < 0: %v", errors4debtus.ErrBadInput, paid)
		return
	}
	bill = inBill
	if bill.ID == "" {
		err = fmt.Errorf("%w: bill.ID is empty string", errors4debtus.ErrBadInput)
		return
	}
	...
	_, changed, index, billMember, billMembers = billAddOrGetMember(bill.Data, groupMember.ID, memberUserID, "", memberUserName)
	...
	if balanceDifference := currentBalance.BillBalanceDifference(previousBalance); balanceDifference.IsNoDifference() {
		...
		if groupChanged, err = applyBillBalanceDifference(splitusSpace.Data, bill.Data.Currency, balanceDifference); err != nil {
...
func DeleteBill(ctx context.Context, billID string, userID string) (bill models4splitus.BillEntry, err error) {
	if err = facade.RunReadwriteTransaction(ctx, func(ctx context.Context, tx dal.ReadwriteTransaction) (err error) {
		if bill, err = GetBillByID(ctx, tx, billID); err != nil {
			return
		}
		...
		if spaceID := coretypes.SpaceID(bill.Data.GetUserGroupID()); spaceID != "" {
			splitusSpace := models4splitus.NewSplitusSpaceEntry(spaceID)
			if err = tx.Get(ctx, splitusSpace.Record); err != nil {
				return err
			}
			var spaceChanged bool
			if spaceChanged, err = splitusSpaceRemoveBill(splitusSpace.Data, bill); err != nil {
				return err
...
func RestoreBill(ctx context.Context, billID string, userID string) (bill models4splitus.BillEntry, err error) {
	if err = facade.RunReadwriteTransaction(ctx, func(ctx context.Context, tx dal.ReadwriteTransaction) (err error) {
		...
		if spaceID := bill.Data.SpaceID; spaceID != "" {
			...
			if groupChanged, err = splitusSpaceAddBill(splitusSpace.Data, bill); err != nil {
				return
...
func InsertBillEntity(ctx context.Context, tx dal.ReadwriteTransaction, billEntity *models4splitus.BillDbo) (bill models4splitus.BillEntry, err error) {
	...
	if billEntity.CreatorUserID == "" {
		err = fmt.Errorf("%w: billEntity.CreatorUserID is empty string", errors4debtus.ErrBadInput)
		return
	}
	if billEntity.AmountTotal == 0 {
		err = fmt.Errorf("%w: billEntity.AmountTotal == 0", errors4debtus.ErrBadInput)
	...
	var key *dal.Key
	if key, err = newBillKey(); err != nil {
		return
	}
	bill.Key = key
	bill.Data = billEntity
	bill.Record = dal.NewRecordWithData(key, billEntity)

	if err = dal4debtus.InsertWithRandomStringID(ctx, tx, bill.Record); err != nil {
		return
	}
	bill.ID = key.ID.(string)
	return
}

//func (billFacade billFacade) createTransfers(ctx context.Context, splitID int64) error {
//	split, err := dal4debtus.Split.GetSplitByID(ctx, splitID)
//	if err != nil {
//		return err
//	}
//	bills, err := dal4debtus.BillEntry.GetBillsByIDs(ctx, split.BillIDs)
//
