...
func (entity *BillsHistoryDbo) BillSettlements() (billSettlements []briefs4splitus.BillSettlementJson) {
	...
	if err := json.Unmarshal([]byte(entity.BillsSettlementJson), &billSettlements); err != nil {
		panic(err)
	}
	return
}
...
func (entity *BillsHistoryDbo) SetBillSettlements(groupID string, billSettlements []briefs4splitus.BillSettlementJson) { // TODO: Enable support for multiple groups
	if data, err := json.Marshal(&billSettlements); err != nil {
		panic(err)
	} else {
...
// NewBillsHistory wraps the data into a BillsHistory record with an incomplete
// key (random string ID generator). The ID is generated client-side on insert
// via dal4debtus.InsertWithRandomStringID.
func NewBillsHistory(data *BillsHistoryDbo) BillsHistory {
...
func NewBillHistoryBillCreated(bill BillEntry, splitusSpaceDbo *SplitusSpaceDbo) (bh BillsHistory) {
	bh = NewBillsHistory(&BillsHistoryDbo{
		...
		GroupIDs:         []string{string(bill.Data.SpaceID)},
	})
...
func NewBillHistoryBillDeleted(userID string, bill BillEntry) BillsHistory {
	return NewBillsHistory(&BillsHistoryDbo{
		...
		TotalAmountAfter:  bill.Data.AmountTotal,
		Action:            BillHistoryActionDeleted,
		BillIDs:           []string{bill.ID},
		GroupIDs:          []string{string(bill.Data.SpaceID)},
	})
}

func NewBillHistoryBillRestored(userID string, bill BillEntry) BillsHistory {
	return NewBillsHistory(&BillsHistoryDbo{
		...
		TotalAmountBefore: bill.Data.AmountTotal,
		TotalAmountAfter:  bill.Data.AmountTotal,
		Action:            BillHistoryActionRestored,
		BillIDs:           []string{bill.ID},
		GroupIDs:          []string{string(bill.Data.SpaceID)},
	})
}
