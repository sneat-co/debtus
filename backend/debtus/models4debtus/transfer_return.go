...
func (t *TransferData) GetReturns() (returns TransferReturns) {
	...
	if err := json.Unmarshal([]byte(t.ReturnsJson), &returns); err != nil {
		panic(err)
	}
...
func (t *TransferData) AddReturn(returnTransfer TransferReturnJson) error {
	...
	if returnTransfer.Time.IsZero() {
		return fmt.Errorf("returnTransfer.ID.IsZero(), ContactID=%v", returnTransfer.TransferID)
	}
	...
	returnsJson, err := json.Marshal(returns) // We'll assign to t.ReturnsJson when all checks are OK
	if err != nil {
