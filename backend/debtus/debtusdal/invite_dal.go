...
func NewInviteDal() InviteDal {
	return InviteDal{}
}

func (InviteDal) GetInvite(ctx context.Context, tx dal.ReadSession, inviteCode string) (invite models4debtus.Invite, err error) {
	if tx == nil {
...
func (InviteDal) ClaimInvite(ctx context.Context, userID string, inviteCode, claimedOn, claimedVia string) (err error) {
	err = facade.RunReadwriteTransaction(ctx, func(tctx context.Context, tx dal.ReadwriteTransaction) error {
		...
		if err = tx.Insert(ctx, inviteClaim.Record, dal.WithAdapterGeneratedID()); err != nil {
			return fmt.Errorf("failed to insert invite claim: %w", err)
		}
		if err := tx.Set(tctx, user.Record); err != nil {
			return fmt.Errorf("failed to save user: %w", err)
		}
		inviteClaimID := inviteClaim.Key.ID.(string)
		logus.Debugf(ctx, "inviteClaimKey.ID: %v", inviteClaimID)
...
func (InviteDal) CreatePersonalInvite(ec strongoapp.ExecutionContext, userID string, inviteBy models4debtus.InviteBy, inviteToAddress, createdOnPlatform, createdOnID, related string) (models4debtus.Invite, error) {
...
func (InviteDal) CreateMassInvite(ec strongoapp.ExecutionContext, userID string, inviteCode string, maxClaimsCount int32, createdOnPlatform string) (invite models4debtus.Invite, err error) {
...
func createInvite(ec strongoapp.ExecutionContext, inviteType models4debtus.InviteType, userID string, inviteBy models4debtus.InviteBy, inviteToAddress, createdOnPlatform, createdOnID string, inviteCodeLen uint8, inviteCode, related string, maxClaimsCount int32) (invite models4debtus.Invite, err error) {
	if inviteCode != AUTO_GENERATE_INVITE_CODE && !dal4debtus.InviteCodeRegex.Match([]byte(inviteCode)) {
		err = fmt.Errorf("invalid invite code: %v", inviteCode)
		return
	}
	if related != "" && len(strings.Split(related, "=")) != 2 {
		err = fmt.Errorf("invalid format for related: %v", related)
		return
	}
	ctx := ec.Context()
	...
	case models4debtus.InviteByEmail:
		if inviteToAddress == "" {
			err = errors.New("email address is not supplied")
			return
		}
		if strings.Index(inviteToAddress, "@") <= 0 || strings.Index(inviteToAddress, ".") <= 0 {
			err = fmt.Errorf("invalid email address: %v", inviteToAddress)
			return
		}
		inviteData.ToEmail = strings.ToLower(inviteToAddress)
		if inviteToAddress != strings.ToLower(inviteToAddress) {
			inviteData.ToEmailOriginal = inviteToAddress
		}
	case models4debtus.InviteBySms:
		...
		inviteData.ToPhoneNumber = phoneNumber
	}
	err = facade.RunReadwriteTransaction(ctx, func(ctx context.Context, tx dal.ReadwriteTransaction) error {
		if inviteCode == AUTO_GENERATE_INVITE_CODE {
			...
			for {
				inviteCode = dal4debtus.RandomCode(inviteCodeLen)
		...
		invite = models4debtus.NewInvite(inviteCode, inviteData)
...
func (InviteDal) ClaimInvite2(ctx context.Context, inviteCode string, invite models4debtus.Invite, claimedByUserID string, claimedOn, claimedVia string) (err error) {
	...
	err = facade.RunReadwriteTransaction(ctx, func(tctx context.Context, tx dal.ReadwriteTransaction) error {
		...
		inviteClaim := models4debtus.NewInviteClaimWithoutID(inviteClaimData)
		if err = tx.Insert(ctx, inviteClaim.Record, dal.WithAdapterGeneratedID()); err != nil {
			return err
		}
		...
		if invite.Data.MaxClaimsCount == 1 {
			...
			counterpartyRecords, err = dal.ExecuteQueryAndReadAllToRecords(ctx, counterpartyQuery, db)
...
func updateUserContactDetails(user dbo4userus.UserEntry, inviteData models4debtus.InviteData) (changed bool) {
	...
	case models4debtus.InviteByEmail:
		...
		email := strings.ToLower(inviteData.ToEmail)
		changed = user.Data.Email != email || !user.Data.EmailVerified
		user.Data.Email = email
		user.Data.EmailVerified = true
		if user.Data.Emails == nil {
			user.Data.Emails = make(map[string]*with.CommunicationChannelProps, 1)
		}
		if props := user.Data.Emails[email]; props == nil {
			emailProps := with.CommunicationChannelProps{
				Type:          "primary",
				Verified:      true,
				CreatedFields: user.Data.CreatedFields,
			}
			if inviteData.ToEmailOriginal != "" {
				emailProps.Original = inviteData.ToEmailOriginal
			}
			user.Data.Emails[email] = &emailProps
			changed = true
	...
	case models4debtus.InviteBySms:
		...
		phoneNumber := strconv.FormatInt(inviteData.ToPhoneNumber, 10)
		if user.Data.Phones == nil {
			user.Data.Phones = make(map[string]*with.CommunicationChannelProps, 1)
		}
		if props := user.Data.Phones[phoneNumber]; props == nil {
			user.Data.Phones[phoneNumber] = &with.CommunicationChannelProps{
				Type:          "mobile",
				Verified:      true,
				CreatedFields: user.Data.CreatedFields,
			}
