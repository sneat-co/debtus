...
func DelayUpdateInviteClaimedCount(ctx context.Context, claimID string) error {
	return delayer4debtus.UpdateInviteClaimedCount.EnqueueWork(ctx, delaying.With(queues.QueueInvites, "UpdateInviteClaimedCount", 0), claimID)
}

func delayedUpdateInviteClaimedCount(ctx context.Context, claimID string) (err error) {
	logus.Debugf(ctx, "UpdateInviteClaimedCount(claimID=%v)", claimID)
	err = facade.RunReadwriteTransaction(ctx, func(ctx context.Context, tx dal.ReadwriteTransaction) (err error) {
		...
		invite, err := dal4debtus.Default.Invite.GetInvite(ctx, tx, claim.Data.InviteCode)
		if err != nil {
			if dal.IsNotFound(err) {
