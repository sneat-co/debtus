...
func ChangeContactStatus(
	ctx facade.ContextWithUser, spaceID coretypes.SpaceID, contactID string, newStatus models4debtus.DebtusContactStatus,
) (
	...
	err = dal4contactus.RunContactusSpaceWorker(ctx, spaceRequest,
...
func createContactWithinTransaction(
	...
	if appUser.Data == nil {
		err = errors.New("appUser.Data == nil")
		return
	}
	if appUser.ID == counterpartyUserID {
		return fmt.Errorf("appUser.ContactID == counterpartyUserID: %v", counterpartyUserID)
	}
	if counterparty.Contact.Data != nil && counterparty.Contact.ID == "" {
		return fmt.Errorf("counterpartyContact.DebtusSpaceContactDbo != nil && counterpartyContact.ContactID == 0: %v", litter.Sdump(counterparty.Contact))
	}
	...
	if creator.DebtusContact, err = dal4debtus.Default.Contact.InsertContact(tctx, tx, creator.DebtusContact.Data); err != nil {
		return
	}
	...
	{ // Verifications for data integrity
		...
		if creator.Contact.Data.UserID != appUser.ID {
			return fmt.Errorf("DebtusContact.UserID != appUser.ContactID: %v != %v", creator.Contact.Data.UserID, appUser.ID)
		}
		if counterparty.Contact.Data != nil {
			if counterparty.Contact.Data.UserID != counterpartyUserID {
				return fmt.Errorf("counterpartyContact.UserID != counterpartyUserID: %v != %v", counterparty.Contact.Data.UserID, counterpartyUserID)
			...
			if creator.DebtusContact.ID == counterparty.Contact.ID {
				return fmt.Errorf("DebtusContact.ContactID == counterpartyContact.ContactID: %v", creator.DebtusContact.ID)
			}
			if creator.Contact.Data.UserID == counterparty.Contact.Data.UserID {
				return fmt.Errorf("DebtusContact.UserID == counterpartyContact.UserID: %v", creator.Contact.Data.UserID)
			}
			...
			if cBalance, cpBalance := creator.DebtusContact.Data.Balance, counterparty.DebtusContact.Data.Balance; !cBalance.Equal(cpBalance.Reversed()) {
				return fmt.Errorf("!DebtusContact.Balance().Equal(counterpartyContact.Balance())\nDebtusContact.Balance(): %v\n counterpartyContact.Balance(): %v", cBalance, cpBalance)
			}
		}
...
func CreateContact(
	ctx facade.ContextWithUser, tx dal.ReadwriteTransaction, spaceID coretypes.SpaceID, contactDetails dto4contactus.ContactDetails,
) (
	contact dal4contactus.ContactEntry,
	contactusSpace dal4contactus.ContactusSpaceEntry,
	debtusContact models4debtus.DebtusSpaceContactEntry,
	...
	if contactIDs, err = dal4debtus.Default.Contact.GetContactIDsByTitle(ctx, tx, spaceID, userID, contactDetails.UserName, false); err != nil {
		return
	}
	spaceRequest := dto4spaceus.SpaceRequest{
		SpaceID: spaceID,
	...
	case 0:
		err = dal4contactus.RunContactusSpaceWorker(ctx, spaceRequest, func(tctx facade.ContextWithUser, tx dal.ReadwriteTransaction, params *dal4contactus.ContactusSpaceWorkerParams) (err error) {
			changes := &createContactDbChanges{
				//user:                user,
				debtusSpace:    models4debtus.NewDebtusSpaceEntry(spaceID),
...
func UpdateContact(ctx context.Context, spaceID coretypes.SpaceID, contactID string, values map[string]string) (debtusSpaceContact models4debtus.DebtusSpaceContactEntry, err error) {
...
func DeleteContact(ctx context.Context, userCtx facade.UserContext, spaceID coretypes.SpaceID, contactID string) (err error) {
...
func GetDebtusSpaceContactsByIDs(ctx context.Context, tx dal.ReadSession, spaceID coretypes.SpaceID, contactsIDs []string) (debtusContacts []models4debtus.DebtusSpaceContactEntry, err error) {
	if tx == nil {
...
func GetDebtusSpaceContactByID(ctx context.Context, tx dal.ReadSession, spaceID coretypes.SpaceID, contactID string) (contact models4debtus.DebtusSpaceContactEntry, err error) {
