...
func (linker *ReceiptUsersLinker) LinkReceiptUsers(ctx context.Context, receiptID, invitedUserID string) (isJustLinked bool, err error) {
	...
	err = db.RunReadwriteTransaction(ctx, func(tctx context.Context, tx dal.ReadwriteTransaction) (err error) {
		...
		if entitiesToSave := changes.Records(); len(entitiesToSave) > 0 {
...
func (linker *ReceiptUsersLinker) linkUsersByReceiptWithinTransaction(
	...
	{ // validate inputs
		...
		if receipt.Data.TransferID != transfer.ID {
			return false, fmt.Errorf("receipt.TransferID != transfer.ContactID: %v != %v", receipt.Data.TransferID, transfer.ID)
		}
		if transferCreatorUserID := transfer.Data.Creator().UserID; transferCreatorUserID == "" {
			return false, errors.New("transfer.Creator().UserID is zero")
		} else if transferCreatorUserID != inviterUser.ID {
			return false, fmt.Errorf("transfer.Creator().UserID != inviterUser.ContactID:  %v != %v", transferCreatorUserID, inviterUser.ID)
		} else if transferCreatorUserID == invitedUser.ID {
			return false, fmt.Errorf("transfer.Creator().UserID == invitedUser.ContactID:  %v != %v", transferCreatorUserID, invitedUser.ID)
		}
	}
	...
	if _, err = GetDebtusSpaceContactByID(tctx, tx, spaceID, transferCreatorCounterparty.ContactID); err != nil {
		...
		return false, fmt.Errorf("inviterContact.UserID !=  inviterUser.ContactID: %v != %v", inviterContact.Data.UserID, inviterUser.ID)
	} else {
	...
	if transfer.Data.DtDueOn.After(time.Now()) {
		if err = dal4debtus.Default.Reminder.DelayCreateReminderForTransferUser(tctx, receipt.Data.TransferID, transfer.Data.Counterparty().UserID); err != nil {
...
func (linker *ReceiptUsersLinker) updateTransfer() (err error) {
	...
	{ // Validate input parameters
		if transfer.ID == "" || transfer.Data == nil {
			return fmt.Errorf("invalid parameter: transfer: %v", transfer)
		}
		validateSide := func(side string, user dbo4userus.UserEntry, contact dal4contactus.ContactEntry, debtusContact models4debtus.DebtusSpaceContactEntry) error {
			if user.ID == "" || user.Data == nil {
				return fmt.Errorf("ReceiptUsersLinker.updateTransfer() => %vUser: %v", side, user)
			}
			if contact.ID == "" || contact.Data == nil {
				return fmt.Errorf("ReceiptUsersLinker.updateTransfer() => %vContact: %v", side, contact)
			} else if contact.Data.UserID != user.ID {
				return fmt.Errorf("ReceiptUsersLinker.updateTransfer() => %vContact.UserID != %vUser.ContactID: %v != %v", side, side, contact.Data.UserID, invited.user.ID)
			}
			return nil
		}
		if err = validateSide("inviter", inviter.user, inviter.contact, inviter.debtusContact); err != nil {
			return
		}
		if err = validateSide("invited", invited.user, invited.contact, invited.debtusContact); err != nil {
			return
		}
		if transfer.Data.CreatorUserID != inviter.user.ID {
			return fmt.Errorf("ReceiptUsersLinker.updateTransfer() => transfer.CreatorUserID != inviterUser.ContactID: %v != %v", transfer.Data.CreatorUserID, invited.user.ID)
		}
	}
	...
	updateTransferCounterpartyInfo := func(
		...
		if counterparty.UserID == "" {
			...
			return fmt.Errorf("updateTransferCounterpartyInfo() => counterparty.UserID != %sUser.ContactID : %s != %s, %sContact.UserID: %s", side, counterparty.UserID, user.ID, side, contact.Data.UserID)
		}
	...
	if err = updateTransferCounterpartyInfo("inviter", transfer.Data.Creator(), inviter.user, invited.contact); err != nil {
		return
	}
	if err = updateTransferCounterpartyInfo("invited", transfer.Data.Counterparty(), invited.user, inviter.contact); err != nil {
		return
	}
